    ...
    return (
        <div className='mt-16 pb-16'>
            ...
            {myOrders.map((order, index) => (
                <div key={index} className='border border-gray-300 rounder-lg mb-10 p-4 py-5 max-w-4xl'>
                    <p className='flex justify-between md:items-center text-gray-400 md:font-medium max-md:flex-col'>
                        <span>OrderId : {order._id}</span>
                        <span>Payment : {order.paymentType} ({order.isPaid ? 'Paid' : 'Pending'})</span>
                    ...
                    {order.items.map((item, index) => (
                        <div key={index} className={`relative bg-white text-gray-500/70 ${order.items.length !== index + 1 && "border-b"} border-gray-300 flex flex-col md:flex-row md:items-center justify-between p-4 py-5 md:gap-16 w-full max-w-4xl`}>
                            {item.product ? (
                                ...
                                <div className='flex items-center mb-4 md:mb-0 text-red-500'>
                                    <p>Product not found (ID: {item.product})</p>
                                </div>
                            )}
                        </div>
                    ))}
